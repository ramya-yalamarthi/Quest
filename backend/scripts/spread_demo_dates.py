"""
One-time fix for the demo corpus: the [k8s] Karpenter reference Cases (seeded
by import_github_issues.py) were all created within the same few days, so the
popup's incident-trend buckets (week/month/quarter) always showed identical
...
This reassigns each [k8s] Case a random age (weighted so week < month <
quarter, growing realistically) and recreates it as a freshly resolved Case
with a new ticket number.

Usage:
    cd backend
    python3 scripts/spread_demo_dates.py
"""
import os
import random
import sys
...
def main(argv):
    ...
    # NOTE: startswith(title,'[k8s]') is broken on this Dataverse instance --
    ...
    # verified to match exactly the 224 "[k8s] ..." Cases with zero false
    # positives, so use that instead.
    import urllib.parse
    params = {
        "$filter": "statecode eq 1 and contains(title,'k8s')",
